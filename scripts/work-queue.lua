...
queue which will call a specified function and use a specified key in global for
the state.  Queues cannot currently be shared or stored in global by the
user--they're singletons.  This may later be fixed with a migration which kills
the work_queues key of global, then starts returning a full state.  That's not
...
queue model.  These items are stored in global and cannot contain functions or
...
-- This state, held outside global, is reconstituted on imports.  It is used to
-- let this module know about all work queues.
---@type fa.WorkQueueHandle[]
local queues = {}
...
function qstate_from_global(name)
   ...
   return global.work_queues[name]
end

function WorkQueueHandle:enqueue(item)
   local state = qstate_from_global(self.name)
...
function mod.on_tick()
   for _, q in pairs(queues) do
      local state = qstate_from_global(q.name)
      local did = 0
