...
function mod.rail_ent_info(pindex, ent, description)  
   ...
   if mod.is_intersection_rail(ent, pindex) then
...
function mod.get_next_rail_entity_ahead(origin_rail, dir_ahead, only_this_segment)
   local next_entity, next_entity_label, result_extra, next_is_forward = mod.identify_rail_segment_end_object(origin_rail, dir_ahead, false, false)
   ...
   if next_entity_label == "train stop" and next_is_forward == false then
      next_entity, next_entity_label, result_extra, next_is_forward = mod.identify_rail_segment_end_object(current_rail, neighbor_r_dir, true, false)
   end
   ...
   while not only_this_segment and next_entity_label == "other rail" and iteration_count < 100 do 
      if iteration_count % 2 == 1 then
         ...
         next_entity, next_entity_label, result_extra, next_is_forward = mod.identify_rail_segment_end_object(current_rail, neighbor_r_dir, false, true)
         --Correction for the train stop exception
         if next_entity_label == "train stop" and next_is_forward == false then
            next_entity, next_entity_label, result_extra, next_is_forward = mod.identify_rail_segment_end_object(current_rail, neighbor_r_dir, true, true)
         end
         ...
         next_entity, next_entity_label, result_extra, next_is_forward = mod.identify_rail_segment_end_object(current_rail, neighbor_r_dir, false, false)
         --Correction for the train stop exception
         if next_entity_label == "train stop" and next_is_forward == false then
            next_entity, next_entity_label, result_extra, next_is_forward = mod.identify_rail_segment_end_object(current_rail, neighbor_r_dir, true, false)
         end
...
function mod.rail_read_next_rail_entity_ahead(pindex, rail, is_forward)
   ...
   local next_entity, next_entity_label, result_extra, next_is_forward, iteration_count = mod.get_next_rail_entity_ahead(origin_rail, dir_ahead, false)
